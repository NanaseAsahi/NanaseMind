...
from model.model_NanaseMind import NanaseMindConfig
from datasets.lm_dataset import PretrainDataset
...
def train_epoch(epoch, loader, iters, start_step=0, wandb=None):
    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step+1):
        ...
        # 把计算好的lr给optimizer用 防止optimizer自动调整lr
        ...
        if (step % args.save_interval == 0 or (step + 1) == iters) and is_main_process():
            model.eval()  # 切换到评估模式
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="NanaseMind Pretraining")
    ...
    parser.add_argument("--wandb_project", type=str, default="NanaseMind-Pretrain", help="wandb项目名")
    ...
    lm_config = NanaseMindConfig(hidden_size=args.hidden_size, num_hidden_layers=args.num_hidden_layers, use_moe=bool(args.use_moe))
